...
from utils.shapley import all_subsets, list2str, cal_shapley, cal_stratified_shapley, shapley_rank


if __name__ == '__main__':
    args = args_parser()
    args.method = "stratified_shapley"
    ...
    with open('./save/Stratified_Shapley.txt', 'a') as f:
        f.truncate(0)
    with open('./save/Margin_contribution_stratifiedshapley.txt', 'a') as f:
        f.truncate(0)
    ...
    for iter in range(args.epochs):
        ...
        print(accuracy_dict)
        shapley_dict = cal_stratified_shapley(accuracy_dict, args.num_users)
        # 边际贡献写入txt文件
        with open('./save/Margin_contribution_stratifiedshapley.txt', 'a') as f:
