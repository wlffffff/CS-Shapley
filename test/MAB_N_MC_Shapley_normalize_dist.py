...
if __name__ == '__main__':
    ...
    for iter in range(args.epochs):
        ...
        if iter == 0:             # 第一轮需要采样所有排列，因此需要聚合所有可能的子模型，并获得其准确率
            ...
            print(SVR_index)
            sample_size = normalization_distribution(SVR_index)     # 用于确定每个分层的采样数量
            print(sample_size)
        else:
            # print(index)
        ...
        # SV写入txt文件
        with open('./save/Mab_N_MC_Shapley_normalize_dist.txt', 'a') as f:
            f.write('Training round:' + str(iter) + '\n')
    ...
    with open('./shapley_result/running_time.txt', 'a') as f:
        f.write('Mab_N_mc_shapley_normalize_dist: client number_' + str(args.num_users) + ' selected strat_' + str(int(args.num_users* args.para_K)) + ' para_H_' + str(args.para_H) + " Para_iteration_" + str(args.iteration) +
