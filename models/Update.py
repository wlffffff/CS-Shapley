...
class LocalUpdate(object):
    ...
    def train_val_test(self, dataset, idxs):
        ...
        trainloader = DataLoader(DatasetSplit(dataset, idxs_train),
                                 batch_size=self.args.bs_train, shuffle=True)
    ...
    def gradient_train(self, net):
        net.train()
        initial_model = copy.deepcopy(net.state_dict())
        updated_model = copy.deepcopy(net.state_dict())
        # print(initial_model['layer_input.weight'])
        # train and update
        optimizer = torch.optim.SGD(net.parameters(), lr=self.args.lr, momentum=self.args.momentum)
        ...
        for iter in range(self.args.local_ep):
            ...
            for batch_idx, (images, labels) in enumerate(self.trainloader):
                ...
                batch_loss.append(loss.item())
            epoch_loss.append(sum(batch_loss)/len(batch_loss))
        # print(net.state_dict()['layer_input.weight'])
        for key in net.state_dict():
            updated_model[key] = net.state_dict()[key] - initial_model[key]
        # print(updated_model['layer_input.weight'])
        return updated_model, sum(epoch_loss) / len(epoch_loss)
